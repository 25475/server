...
async function seedData() {
    ...
    const productsData = [
        // Productos Vision
        {
            ...
            price: 89.99,
            imageUrl: 'https://via.placeholder.com/400x300?text=Camara+IP+Domo',
        ...
        },
        {
            ...
            price: 129.99,
            imageUrl: 'https://via.placeholder.com/400x300?text=Camara+IP+Bullet',
            categoryId: categories.find(c => c.slug === 'camaras-ip')?.id,
            productType: 'VISION',
        ...
        // Productos Novatec
        {
            ...
            price: 899.99,
            imageUrl: 'https://via.placeholder.com/400x300?text=PC+Gamer',
        ...
        },
        {
            ...
            price: 799.99,
            imageUrl: 'https://via.placeholder.com/400x300?text=Laptop+Dell',
            categoryId: categories.find(c => c.slug === 'computadoras')?.id,
            productType: 'NOVATEC',
    ...
    const clientImagesDir = path_1.default.join(__dirname, '../../src/Image/Productos');
    ...
    let clientFiles = [];
    try {
        clientFiles = fs_1.default.existsSync(clientImagesDir) ? fs_1.default.readdirSync(clientImagesDir) : [];
    }
    catch (err) {
        clientFiles = [];
    }
    ...
    for (const f of clientFiles) {
        ...
        if (!['.png', '.jpg', '.jpeg', '.webp', '.svg'].includes(ext))
            continue;
        const base = path_1.default.basename(f, ext);
    ...
    for (const prod of productsData) {
        try {
            if (!prod.imageUrl) {
                ...
                if (matched) {
                    const found = path_1.default.join(clientImagesDir, matched);
                    const ext = path_1.default.extname(found).toLowerCase();
                    const base = path_1.default.basename(found, ext);
                    const normFilename = `${normalize(base)}${ext}`;
                    const dest = path_1.default.join(uploadsDir, normFilename);
    ...
    const plans = await Promise.all(plansData.map((plan, index) => prisma.plan.upsert({
        where: { id: `plan-${index}` },
        update: {},
        create: plan,
    })));
